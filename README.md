...
The **Fresh Entry V3** model is separate from the strength ranking. It first requires persistent real strength: 10-session Leadership/Long-Momentum/Sector confirmation, positive 60/120-session relative strength versus VN-Index and sector, a rising medium-term MA50/MA100 structure, and confirmation from the previous completed weekly trend. Only then can an entry trigger occur, either through a controlled pullback/resumption near MA20 or a breakout from a recent Bollinger squeeze. MA20/MA50 crosses and one-day momentum threshold crosses are no longer standalone entry triggers because they are too lagging and chase-prone. Default anti-chase limits are 8% above MA20, +10% over five sessions, two sessions from entry, and +5% above the recorded entry price. Entry events remain versioned in Supabase.
